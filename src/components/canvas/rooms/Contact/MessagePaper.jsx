...
const analyzeContentAI = (text, isSubject = false) => {
    if (!text || text.trim().length < (isSubject ? 2 : 3)) return { isSpam: true, reason: '留言过短' };
    ...
    if (!isSubject && cleaned.length <= 15 && !cleaned.includes(' ')) {
        ...
        const commonShort = /^(hi|hey|hello|thanks|thank you|cool|nice|ok|okay|yes|no|sup|yo|cheers|hej|cześć|dzięki|siema|elo|你好|嗨|谢谢|感谢|哈喽|哈罗)$/i;
        if (!commonShort.test(cleaned)) {
            return { isSpam: true, reason: '内容过短，不像是有效留言' };
        }
    }
    ...
    if (scorableWords.length === 0) return { isSpam: true, reason: '未检测到有效词汇' };
    ...
    if (avgScore < 0.2) {
        return { isSpam: true, reason: '内容疑似乱码' };
    }
    if (gibberishRatio >= 0.6 && scorableWords.length >= 2) {
        return { isSpam: true, reason: '无法识别的词汇过多' };
    }
    ...
    if (allAlpha.length > 8 && allVowels.length / allAlpha.length < 0.12) {
        return { isSpam: true, reason: '字符分布异常' };
    }
...
const MessagePaper = ({ position = [0, 0.05, 2], onSend }) => {
    ...
    const validateForm = () => {
        ...
        else if (!isValidEmail(email)) newErrors.email = '邮箱格式无效';
    ...
    const handleButtonClick = useCallback(async () => {
        ...
        try {
            ...
            if (subjectAnalysis.isSpam || messageAnalysis.isSpam) {
                setErrors({ message: '我们的 AI 系统将其标记为垃圾信息，请清晰书写。' });
                setIsSubmitting(false);
                return;
            }
            ...
            <>
                ...
                {Object.keys(errors).length > 0 && (
                    <Text
                        ...
                        anchorY="middle"
                    >
                        {errors.email || errors.subject || errors.message || '请填写所有字段'}
                    </Text>
                )}
